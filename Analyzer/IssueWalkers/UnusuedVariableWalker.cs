...
namespace Compiler.IssueWalkers
{
    ...
    internal class UnusedVarWalker : DefaultWalker
    {
        private List<VariableDeclaratorSyntax> declarations;
        ...
        public override void VisitVariableDeclarator(VariableDeclaratorSyntax node)
        {
            ...
            //Console.WriteLine(node.Identifier.ToString() + " at " + position.filepath + " line " + position.lineNumber);
        ...
        {
            int index = declarations.FindIndex(s => s.Identifier.ToString().Equals(node.Identifier.ToString()));
            if(index != -1)
            {
        ...
        internal override void PreExecute() {
            ...
            //IssueReporter.Instance.EnableIssueType(IssueType.UnusedFieldsOrLocalVariables);

            // If you need to execute more code before visiting the AST
            declarations = new List<VariableDeclaratorSyntax>();
        }
        ...
        internal override void PostExecute() {
            ...
            declarations.ForEach(node => {
                var issue = new Issue(IssueType.UnusedFieldsOrLocalVariables, node);
                IssueReporter.Instance.AddIssue(issue);
            });
